...
**With Claude Projects (knowledge files):** If your LIMO files are uploaded as knowledge files in a Claude Project, they can't be edited in place. Use append-only versioning (`STATUS_SNAPSHOT_v2.md`) — see "File Versioning" in `LIMO_FRAMEWORK.md`.
...
**Versioning workaround:** Since ChatGPT can't edit in place, use append-only versioning: when a file needs updating, have the AI generate a new version (`STATUS_SNAPSHOT_v2.md`) and upload it alongside the old one. The AI reads the highest-numbered version. See "File Versioning" in `LIMO_FRAMEWORK.md` for details.
