...
| `BOOT_PACK.md` | Generated (v2.5): a 60-line cold start plus a signpost index — read this first, open the rest on demand |
| `OWNERS.md` | Who may write which file when more than one agent works the domain (v2.4) |
| `TRIGGER_INDEX.md` | Generated (v2.5): keyword → gate map, so a NEVER_AGAIN fires when a request matches, not only at boot |
| `CONTEXT_RESTORE_PROTOCOL.md` | Legacy since v2.4 — its rules now live in the framework's Layer 1 |
...
## What's new in v2.5 (September 2026)

A survey of 29 comparable systems (memory banks for coding agents, memory services, academic architectures) found the field had
converged on LIMO's orientation model and moved memory into databases — except the MemGPT lineage, which moved to git-backed markdown.
Eight mechanisms the field does better were adopted, without changing any file format:

- **Pull-based boot** — `BOOT_PACK.md` is a hard-capped 60-line Part A plus a signpost index; bodies are opened on demand.
- **Trigger injection** — `Triggers:` lines on gates; `limo_lint.py triggers "<request>" Domain` surfaces the ones that match.
- **Line validity and supersession** — `{valid: a..b}`, `{supersedes:}`, `{class:}`, `{confidence:}`; never delete, close the range.
- **Reconcile on write + reflect** — ADD / UPDATE / NOOP before appending; `reflect` proposes merges into `_pending/`, a human promotes.
- **Interruption-driven write-back** — hooks for session start, prompt, pre-compaction and session end (`tools/hooks/`).
- **Ready / claim / stall** — `{blocked_by}` `{claimed}` `{started}` tags on open items; `ready` computes what is actionable.
- **Git as write-ahead log** — the `[Git]` level: a memory-only repository per domain, marker-gated agent commits, stale-lock recovery.
- **Cross-artifact lint** — undefined IDs, GOALS↔STATUS coherence, dead ARCHITECTURE paths.

Four conformance levels: **Core** (markdown only) → **Git** (+ history) → **Agents** (+ ownership, seats, messaging) → **Infra** (+ events, semantic recall, bus).
Each is a superset of the one before. The tool: `python3 tools/limo_lint.py --help`.
...
```
tools/
  transcript_viewer.html → Browse Claude Code / Cowork audit.jsonl transcripts
```

**[Session Transcript Viewer](tools/transcript_viewer.html)** — A standalone HTML file that renders Claude Code and Cowork `audit.jsonl` session transcripts in a searchable chat UI. Filters by message type (User, Assistant, Tool calls, System), full-text search, pagination for large files. Runs entirely in your browser — no data uploaded anywhere. Useful because Cowork compacts long sessions, removing earlier messages from the GUI. The `audit.jsonl` file is the only complete record, and this viewer makes it readable.
