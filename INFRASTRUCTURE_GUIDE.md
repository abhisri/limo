...
> **mem0 2.x note (2026-09):** mem0 2.x has no graph backend. It does not read or write Neo4j; entity linking is a
> recall-ranking boost only. Treat mem0 as the *semantic* store ("vibes") and Neo4j as the *entities and events* store
> ("nouns and verbs"), written deliberately or through an events endpoint. In production this runs as a small FastAPI wrapper
> around `mem0ai` 2.0.x with `pgvector` as the vector store (Option B in `infra/docker-compose.yml`); `search()` / `get_all()`
> in 2.x take entity ids via `filters={...}`, so keep your REST wrapper's contract stable and adapt inside it.
